...
class PatternGenerator {
  ...
  generateRandomPattern() {
    const baseShapeType = this.randomChoice(this.baseShapes);
    const symmetryGroup = this.randomChoice(this.symmetryGroups);
    const baseShape = this.createBaseShape(baseShapeType);
    
    const deformationCount = Math.floor(Math.random() * 3) + 1;
    const deformations = this.generateRandomDeformations(deformationCount, baseShape.points.length);
    
    const escherShape = createEscherLikeShape(baseShape, deformations);
    
    return new TessellationPattern(escherShape, symmetryGroup);
  }

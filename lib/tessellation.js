...
class TessellationPattern {
  ...
  generateHexagonalTiles(width, height) {
    ...
    // For a regular hexagon, the distance between opposite vertices is 2*radius
    // The distance between opposite flat sides (width) is radius * sqrt(3)  
    const hexRadius = (bounds.maxX - bounds.minX) / 2;
    
    // For perfect hexagonal tessellation (honeycomb pattern)
    // Horizontal distance between hexagon centers should be width of hexagon
    // which is the distance between parallel sides = radius * sqrt(3)
    const hexWidth = bounds.maxX - bounds.minX;
    // Vertical distance between row centers is 3/4 of hexagon height
    const hexHeight = (bounds.maxY - bounds.minY) * 0.75;
    ...
    for (let row = -1; row < rows; row++) {
      for (let col = -1; col < cols; col++) {
        // Alternate rows are offset by half the horizontal spacing
        const offsetX = col * hexWidth + (row % 2) * hexWidth / 2;
        const offsetY = row * hexHeight;

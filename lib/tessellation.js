...
class TessellationPattern {
  ...
  generateHexagonalTiles(width, height) {
    ...
    const hexWidth = hexRadius * 1.5;  // Horizontal spacing between hexagon centers
    const hexHeight = hexRadius * Math.sqrt(3);  // Vertical spacing between rows
    ...
    for (let row = -1; row < rows; row++) {
      for (let col = -1; col < cols; col++) {
        const offsetX = col * hexWidth;
        const offsetY = row * hexHeight + (col % 2) * hexHeight / 2;
